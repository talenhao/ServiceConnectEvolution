                    ...
                    if connections:
                        ...
                        for connection in connections:
                            ...
                            if connection.status == psutil.CONN_LISTEN:
                                ...
                                pLogger.debug("CONN_LISTEN connection is {!r}".format(connection))
                                process_listen_port.add(connection.laddr[1])
                                pLogger.debug("connection.laddr is {!r}".format(connection.laddr))
                                # none LISTENS process
                            else:
                                pLogger.warn("{!r} is not LISTEN connection!".format(connection))
                        # take out from set.
                        ...
                        for connection in connections:
                            ...
                            if connection.status != psutil.CONN_LISTEN \
                                ...
                                pLogger.warn("{!r} is not our collect connection!".format(connection))
                        ...
                        if process_connection_ip_port:
                            ...
                            pLogger.warn("process_connection_ip_port is empty!")
                    else:
...
def import2db(table, ip, port, p_name, p_pid, p_exe, p_cwd, p_cmdline, p_status, p_create_time, p_username,
    ...
    try:
        if table == service_listens_table:
            ...
            port_column = "l_port"
            columns_num = 11
            pLogger.debug("Table is {}, column_num is {}".format(service_listens_table, columns_num))
            ...
            port_column = "c_port"
            columns_num = 12
            pLogger.debug("Table is {}, column_num is {}, append column.".format(service_listens_table, columns_num))
